...
int solve(int tc) {
	...
	F0R(i,N) {
		// dbg("TRYING",i);
		correct[i] = 0;
		F0R(j,M) correct[i] += cor[i][j] == '1';
		// dbg("HA",sum);
		db skill = predict[correct[i]];
		db skill_cheat = predict[min(max(2*(correct[i]-5000),0),10000)];

		db prob_good = 0, prob_cheat = 0;
		F0R(j,M) {
			db p_ok = 1/(1+exp(difficulty[j]-skill));
			db p_wrong = 1-p_ok;
			db p_ok_cheat = 1/(1+exp(difficulty[j]-skill_cheat));
			db p_wrong_cheat = 1-p_ok_cheat;
			// dbg("??",p_ok,p_wrong,p_ok_cheat,p_wrong_cheat);
			if (cor[i][j] == '1') {
				prob_good += log(p_ok);
				prob_cheat += log((1+p_ok_cheat)/2);
			} else {
				prob_good += log(p_wrong);
				prob_cheat += log(p_wrong_cheat/2);
			}
		}

		prob[i] = prob_cheat-prob_good;
		// if (i == cheat) {
		// 	dbg("??",prob_cheat,prob_good);
		// }
		// prob[i] -= expect;
		// expected log: 
	...
	int bes = -1;
	// if (correct[i] >= 6000) 
	F0R(i,N) if (bes == -1 || prob[i] > prob[bes]) bes = i;
	// // if (cheadbg("??",bes,correct[bes]);
	// // dbg("??",prob[cheat],prob[bes]);
	return bes;
}
...
void test() {
	...
	F0R(j,M) y[j] = rng_db(-3,3);
	cheat = rng()%N;
	F0R(i,N) {
		cor[i] = str(M,'0');
		F0R(j,M) {
			if (i == cheat && rng_db(0,1) < 0.5) cor[i][j] = '1';
			else if (rng_db(0,1) < 1/(1+exp(x[i]-y[j]))) cor[i][j] = '1';
		}
	}
	int bes = solve(0);
	++num;
	...
	dbg(cheat,bes,prob[cheat],prob[bes],num,ok);
	dbg("??",cheat,correct[cheat],correct[bes]);
	// sor(prob);
	// dbg(prob);
	// solve(0);
}
